import { CrashState, PlayerState, AmbulanceState, AmbulancePhase } from "./types";
import {
    INITIAL_SPEED,
    CRASH_DURATION,
    CRASH_SHAKE_INITIAL,
...
export function initCrashPhysics(cs: CrashState, p: PlayerState, speed: number): void {
...
export function updateCrashPhysics(cs: CrashState, dt: number, rawDt: number, groundY: number): boolean {
...
export function createAmbulanceState(playerX: number, playerWidth: number, canvasW: number, groundY: number): AmbulanceState {
    return {
        x: canvasW + 140,
...
export type AmbulanceAction = "none" | "stop_siren" | "revive" | "resume_game" | "remove";

export function updateAmbulanceLogic(amb: AmbulanceState, dt: number, rawDt: number, canvasW: number): AmbulanceAction {
